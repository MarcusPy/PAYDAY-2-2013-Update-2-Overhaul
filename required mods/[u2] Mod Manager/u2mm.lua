...
function u2mm:matchSavedValueToOption(val, data, simple)
	...
	if dataCopy.type == "CoreMenuItemSlider.ItemSlider" then
		if val >= dataCopy.min and val <= dataCopy.max then
			return val
		end
	end
	
    for idx, option in ipairs(dataCopy) do
        if type(option.value) == "string" and u2mm:startsWith(option.value, "{") then --currently retired, loadstring method is way too convoluted
			log("u2mm:matchSavedValueToOption invoked with string value, switch to idx table + _G table corresponding values if possible")
