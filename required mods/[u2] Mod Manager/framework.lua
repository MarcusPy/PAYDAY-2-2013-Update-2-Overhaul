...
if RequiredScript == "lib/managers/menumanager" then
	...
	-- DO NOT set any value to 0, it will cause the game to freeze, no idea why this happens
	...
	function u2mm:addSlider(name, title, desc, cbk, value, parent, data)
		local params = {
			name = name,
			text_id = title,
			help_id = desc,
	...
	function u2mm:addModManagerMenu()
		...
		if u2mm.origin.main then
			u2mm.origin.main:insert_item(optionsButton, #u2mm.origin.main._items - 1) --where and at what index do we want to insert, or use add_item(optionsButton) to append
			u2mm.origin.main:delete_item("gamehub")
			u2mm.origin.main:delete_item("credits")
			u2mm.origin.main:delete_item("divider_test2")
			u2mm.origin.main:delete_item("divider_test2")
		else
			u2mm.origin.pause:insert_item(optionsButton, #u2mm.origin.pause._items - 1)
		end
	end
end