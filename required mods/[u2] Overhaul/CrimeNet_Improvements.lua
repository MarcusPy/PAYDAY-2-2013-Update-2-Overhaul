if RequiredScript == "lib/managers/crimenetmanager" then
    ...
    local old_create_job_gui = CrimeNetGui._create_job_gui
    ...
    function CrimeNetGui:init( ws, fullscreeen_ws, node, ... )
        ...
        if (_G.u2_core.settings.crimenet_visuals.crimenet_background_colour.enabled == true) then
            map:set_color(_G.u2_core.settings.crimenet_visuals.crimenet_background_colour.color)
        end
    end

    function CrimeNetGui:_create_locations(...)
        old_create_locations(self, ...)
        if (_G.u2_core.settings.crimenet_heists.crimenet_sorted_heist_blips == true) then
            local newDots = {}
            local xx,yy = 12,10
            for i=1,xx do
                for j=1,yy do
	...
	function CrimeNetGui:_create_job_gui(data, type, fixed_x, fixed_y, fixed_location, ...)
		...
		local result = old_create_job_gui(self, data, type, fixed_x, fixed_y, fixed_location, ...)
		tweak_data.menu.pd2_small_font_size = size
        if (_G.u2_core.settings.crimenet_heists.crimenet_risk_colour == true) then
            if result.side_panel and result.side_panel:child('job_name') then
                local colors = { Color.green, Color.yellow, Color.red }
                result.side_panel:child('job_name'):set_color(colors[data.difficulty_id - 2] or Color.white)
            end
        end
		return result
	end
	...
	function CrimeNetGui:update_server_job(data, i, ...)
        ...
        if (_G.u2_core.settings.crimenet_heists.crime_net_risk_colour[1] == true) then
		    job.side_panel:child("job_name"):set_color(_G.u2_core.settings.crimenet_heists.crimenet_risk_colour[2][data.difficulty_id - 2] or Color.white)
        end
	end
end