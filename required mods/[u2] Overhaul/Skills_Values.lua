...
function UpgradesTweakData:_init_pd2_values()
	...
	--[[ A R M O R ]]
	--suit, light, vest, vest collar, jacket, set, ictv
	-- ictv requires a high lvl perk, so as a bonus it doesnt slow you down more than set
	self.values.player.body_armor = { 1, 3, 5, 7, 9, 12 }
	self.values.player.armor_movement_penalty = { 1, 0.95, 0.9, 0.85, 0.8, 0.8 } -- A multiplier on movement speed
	...
	self.values.carry.throw_distance_multiplier						= { 1.5 }		-- ACE
			...
			-- AMMO_RESERVOIR
	self.values.temporary.no_ammo_cost										= { {true, 14}, {true, 14} } -- BASIC, ACE. { value, time }
	...
	--[[ T E C H N I C I A N ]]
		--[[ TIER 1 ]]
			-- RIFLEMAN
	self.values.assault_rifle.enter_steelsight_speed_multiplier	= { 1.33 }	-- BASIC
	self.values.assault_rifle.zoom_increase								= { 1.2 }		-- ACE. increase in the tweak_data.weapon.stats.zoom
	
			-- MASTER_CRAFTSMAN
	self.values.player.crafting_weapon_multiplier					= { 0.9 }		-- BASIC
	...
	self.values.player.silent_kill												= { 300 }		-- ACE. in cm alert radius on dead and hurt enemies
	...
	self.values.player.pick_lock_easy_speed_multiplier		= { 0.6 }	-- BASIC. reduces speed for pick_lock_easy, pick_lock_easy_no_skill and pick_lock_hard_no_skill interactions
