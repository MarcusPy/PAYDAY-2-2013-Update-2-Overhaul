...
if RequiredScript == "lib/managers/menu/menuscenemanager" then
	function MenuSceneManager:_set_dimensions()
		...
		local y = (1-(aspect_ratio / (ratios[uwf.settings.aspect_ratio][1]/ratios[uwf.settings.aspect_ratio][2])))/2
		local h = aspect_ratio / (ratios[uwf.settings.aspect_ratio][1]/ratios[uwf.settings.aspect_ratio][2])
	...
	function MenuSceneManager:setup_camera()
		...
		self._vp:camera():set_width_multiplier(CoreMath.width_mul( ratios[uwf.settings.aspect_ratio][1]/ratios[uwf.settings.aspect_ratio][2] ) )
...
if RequiredScript == "lib/managers/menu/menubackdropgui" then
	-- makes briefing more UW friendly
	function MenuBackdropGUI:_create_base_layer()
		...
		local aspect_ratio = ratios[uwf.settings.aspect_ratio][1] / ratios[uwf.settings.aspect_ratio][2]
		bd_base_layer:set_size(750 * aspect_ratio, 720)
		--bd_base_layer:set_size(3840, 1080)
	end
	...
	function MenuBackdropGUI:set_pattern(bitmap_texture, alpha, blend_mode)
		...
		self:set_fullscreen_bitmap_shape(object, 1.75) --makes the graphic in the back larger
		...
		local function mechanic_animation(o)
			...
			while true do
				...
				over(math.abs(diff / 30), move_one_axis, self._fixed_dt)
				over(math.abs(overshoot / 30), overshoot_one_axis, self._fixed_dt)
				wait(math.rand(0.1), self._fixed_dt)
